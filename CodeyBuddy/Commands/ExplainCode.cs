...
{
    [Command(PackageIds.ExplainCode)]
    internal sealed class ExplainCode : BaseCommand<ExplainCode>
    {
        protected override async Task ExecuteAsync(OleMenuCmdEventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        throw new Exception("Please supply the prompt/text for explanation");
                    }
                }
            ...
            {
                await VS.MessageBox.ShowWarningAsync("CodeyBuddy ERR003 :- " + ex.Message);
        ...
        private async Task InvokeAPIAsync(string prompt)
        {
            ...
            if (!string.IsNullOrEmpty(prompt))
            {
                using (Utilities utilities = new Utilities())
                {
                    ...
                    output = await utilities.InvokeOpenAIAPIAsync(prompt, "Explain");
